...
decompress(source, tempDir)
	.then((files) => {
		...
		if (!existsSync(timeFile)) {
			writeFileSync(
				timeFile,
				`${Number(process.env.COMMIT_TIME) * 1000 || Date.now()}`,
			);
		}
		...
		command(
			`find . -exec touch -t "${date.toISOString().substring(0, 16).replace(/\D/g, '')}" '{}' +`,
			{
				cwd: tempDir,
				env: {
					...process.env,
		...
		filePaths.forEach((filePath) => {
			command(
				`zip -rq -J -D -X -9 --compression-method deflate ../${dest} ${filePath}`,
				{cwd: tempDir},
			);
		});
