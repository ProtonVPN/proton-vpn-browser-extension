...
export const withSearchScore = (
	...
	each(source.groups || {}, (key, value) => {
		const filteredGroup = withSearchScore(
			value,
			countryScore,
			searchWords,
			useCountryScore,
		...
		if (
			filteredGroup.searchScore ||
			filteredGroup.logicals?.length ||
		...
		...(useCityScore && source.logicals
			? {
					logicals: source.logicals
						...
						.map((logical) => ({
							...
							SearchScore: getSearchScoreForWords(searchWords, [
								...
								...Object.entries(logical.Translations || {}).reduce(
									(list, [key, sentence]) =>
										useCountryScore || key !== 'EntryCountryName'
						...
						.filter(
							(logical) =>
								countryScore || logical.SearchScore >= searchWords.length,
						),
				}
			: {}),
