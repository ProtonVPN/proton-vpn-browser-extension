...
const serverRotatorStore = storage.item<ServerRotatorState>(
	'server-rotator',
	Storage.LOCAL,
);

export const getServerRotatorState = async (): Promise<ServerRotatorState> => {
	return ((await serverRotatorStore.get()) ??
		{}) as Promise<ServerRotatorState>;
};

export async function setServerRotatorState(
	partialState: Partial<ServerRotatorState>,
): Promise<ServerRotatorState> {
