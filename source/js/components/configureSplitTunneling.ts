import type {StoredWebsiteFilterList} from '../vpn/WebsiteFilter';
...
const updateDropdownUI = (area: ParentNode) => {
	...
	const modeDropdownMenu =
		area.querySelector<HTMLDivElement>('#modeDropdownMenu')!;
	const includeModeTick =
...
export const configureSplitTunneling = (
	feature: LoadedFeature<SplitTunneling>,
	list: StoredWebsiteFilterList | undefined,
	area: ParentNode,
	...
	area
		.querySelectorAll<HTMLDivElement>('[data-page="split-tunneling"]')
		.forEach((page) => {
			page.classList[feature.feature.isControllable() ? 'remove' : 'add'](
				'locked-feature-page',
			);
			page
				.querySelectorAll<HTMLParagraphElement>(
					'.locked-feature-page-warning p',
				)
				.forEach((paragraph) => {
					paragraph.innerText = feature.feature.getControlMessage();
				});
		});
	...
	const storeList = () => {
		if (!feature.feature.isControllable()) {
			return;
		}
	...
	const renderDomainList = () => {
		const html = domainManager
			...
			.map((exclusion, index) => {
				...
				return `<div class="flex my1">
					<div class="flex-1 ov-hidden"><div class="ellipsis">${domain}</div>${
						exclusion.withSubDomains
							? `<div class="fade-text small-text">${
		...
		area
			.querySelectorAll<HTMLDivElement>(
				'.split-tunneling-filters .domains-list',
			)
			.forEach((element) => {
				...
				element.addEventListener('click', (e) => {
					...
					if (button) {
						const indexToDelete = Number(
							button.getAttribute('data-st-delete-index'),
						);
	...
	const toggleAddForm = (open: boolean) => {
		area
			.querySelectorAll<HTMLDivElement>('.split-tunneling-filter-add')
			.forEach((element) => {
				element.style.display = open ? 'none' : 'block';
			});

		area
			.querySelectorAll<HTMLDivElement>('.split-tunneling-filter-form')
			.forEach((element) => {
				...
				element
					...
					.forEach((cancelButton) => {
						cancelButton.disabled =
							domainManager.getCurrentModeDomainsCount() === 0;
					});
			});

		area
			.querySelectorAll<HTMLDivElement>('.split-tunneling-filters')
			.forEach((element) => {
	...
	const refreshEnabled = () => {
		...
		area
			...
			.forEach((configurationBlock) => {
				...
				const selectedMode =
					switchBar.querySelector<HTMLDivElement>('#selectedMode');
				const selectedModeText =
					switchBar.querySelector<HTMLSpanElement>('#selectedModeText');
				const modeDropdownMenu =
					switchBar.querySelector<HTMLDivElement>('#modeDropdownMenu');
				const modeOptions =
					modeDropdownMenu?.querySelectorAll<HTMLDivElement>(
						'.dropdown-option',
					);
				...
				configurationBlock
					.querySelectorAll<HTMLDivElement>('.domains-list-warning')
					.forEach((block) => {
						block.style.display =
							isIncludeMode && isBrave(navigator) ? 'block' : 'none';
					});
				...
				modeOptions.forEach((option) => {
					...
					option.addEventListener('click', () => {
						const selectedValue = option.getAttribute(
							'data-value',
						) as keyof typeof SplitTunnelingMode;
	...
	if (upgradeNeeded) {
		document
			?.querySelectorAll<HTMLInputElement>('[data-go-to="split-tunneling"]')
			.forEach((button) => {
	...
	const checkValidity = () => {
		area
			.querySelectorAll<HTMLButtonElement>('[data-st-action="add"]')
			.forEach((button) => {
	...
	const checkbox = area.querySelector<HTMLInputElement>(
		'.with-subdomains',
	) as HTMLInputElement;
	...
	area
		...
		.forEach((previousButton) => {
			...
			(previousButton.parentNode as ParentNode).replaceChild(
				button,
				previousButton,
			);
