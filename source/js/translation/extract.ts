...
const [mode] = process.argv.slice(
	extractFile ? process.argv.indexOf(extractFile) + 1 : 2,
);
...
const dumpFile = (): string => {
	if (mode === 'po') {
		...
		getKeys(outputData)
			.sort(compare)
			.forEach((key) => {
				getKeys(outputData[key])
					.sort(compare)
					.forEach((id) => {
						translations.push(
							(outputData[key] as Record<string, Translation>)[
								id
							] as Translation,
						);
					});
	...
	getKeys(outputData)
		...
		.forEach((key) => {
			...
			getKeys(outputData[key])
				...
				.forEach((id) => {
					translations[id] = (
						(outputData[key] as Record<string, Translation>)[id] as Translation
					).values;
				});
...
Promise.all([
						...
						(file) =>
							new Promise((fileResolve) => {
								readFile(appDirectory + '/' + file, (err, content) => {
											...
											.forEach((element) => {
												...
												if (attributeValue) {
													addTranslation(
														...
														[element.getAttribute('data-comment')].filter(
															Boolean,
														) as string[],
													);
												}
									...
									root
										.querySelectorAll('[data-tooltip-title]')
										.forEach((element) => {
											const titles = element.getAttribute('data-tooltip-title');

											if (titles) {
												(Object.values(JSON.parse(titles)) as string[]).forEach(
													(title) => {
														addTranslation(
															outputData,
															file,
	...
	new Promise((globResolve) => {
		...
		const translatePath = realpathSync(
			appDirectory + '/js/tools/translate.ts',
		).replace(/\.ts$/, '');
					...
					files.map(
						(filePath) =>
							new Promise((fileResolve) => {
								...
								const getImportPath = (libraryName: string) =>
									realpathSync(
										baseDirectory + libraryName.replace(/\.ts$/, '') + '.ts',
									).replace(/\.ts$/, '');
								parser
									.parseFile(fullPath, appDirectory + '/..')
									.then((file) => {
										...
										file.imports.forEach((namedImport) => {
											if (
												namedImport instanceof NamedImport &&
										...
										if (translateFunction || contextFunction) {
											...
											if (!sourceFile) {
												throw new Error(
													'Source file not found for ' + fullPath,
												);
											}
											...
											const scanNode = (
												...
												if (isCallExpression(node)) {
													...
													if (textNode) {
														...
														const referenceNode =
															textNode instanceof ContextualizedTranslation
																? node
																: textNode;
														const position =
														...
														addTranslation(
															...
															[
																...(getLeadingCommentRanges(
																	...
																	fullText.substring(node.pos, node.end),
																) || []),
																...(getTrailingCommentRanges(
																	fullText,
																	node.getFullStart(),
	...
	writeFile(
		appDirectory + '/locales/en_US.' + (mode || 'json'),
		dumpFile(),
		(err) => {
			if (err) {
				throw err;
