import type {ApiError, DeviceLimitErrorDetails, ErrorAction} from '../api';
import {
	ErrorActionCategory,
	ErrorActionCode,
	isDeviceLimitError,
...
function getActionAttributes(action: ErrorAction): string | null {
	if (action.URL) {
		return `href="${action.URL.replace(/^\//, accountURL)}"`;
	}

	if (
		action.Code === ErrorActionCode.SIGN_OUT ||
		action.Code === ErrorActionCode.REFRESH
	) {
		return `href="#" data-action="${action.Code}"`;
	}
...
function getActionLink(action: ErrorAction): string {
	const attributes = getActionAttributes(action);
	...
	const classes = [
		(
			...
			} as Record<ErrorAction['Category'], string>
		)[action.Category] || '',
		(
			{
				[ErrorActionCode.UPGRADE]: 'upgrade-button',
			} as Partial<Record<ErrorActionCode, string>>
		)[action.Code] || '',
	]
		.filter(Boolean)
		.join(' ');

	return `<a class="${classes}" ${attributes}>${action.Name || c('Action').t`Upgrade`}</a>`;
}

export function getErrorAsString(error: unknown, allowHtml = false): string {
	const message =
		(error as ApiError).Error || (error as Error).message || error;
	...
	if (/\[object/.test(strMessage)) {
		...
		if (!allowHtml || keys.length === 0 || keys.join(',') === '_id') {
			return c('Error')
				.t`Something went wrong. If the problem persists, please try to re-install the extension`;
		}
...
function getFeatureIconAndText(icon: string, text: string): string {
	const svgContent =
		{
	...
	return `
		<div class="feature-row">
			...
			${
				svgContent
					? `<svg class="feature-icon" viewBox="0 0 24 24">${svgContent}</svg>`
					: ''
			}
			</div>
...
function parseErrorBody(
	body: DeviceLimitErrorDetails['Body'],
): string | undefined {
	...
	return body
		.map((component) => {
			switch (component.Component) {
				case 'Feature':
					return getFeatureIconAndText(component.Icon, component.Text);
...
export function getErrorMessage(error: ApiError | Error | ErrorDump) {
	if (needsUpdate(error) || isDeviceLimitError(error)) {
		...
		const parsedBody = parseErrorBody(
			(apiError.Details as DeviceLimitErrorDetails)?.Body,
		);
		const body =
			parsedBody ||
			(errorCode === 86151
		...
		const errorFallback =
			apiError.Details?.Title || apiError.Details?.Hint || parsedBody
				? ''
		...
		return {
			...
			blockingError: `
				...
				<div class="text-center" style="padding: 20px; line-height: 1.4em;">
					...
					${((apiError.Details as DeviceLimitErrorDetails)?.Actions || []).map(getActionLink).join('')}
				</div>
			`,
		};
	}
