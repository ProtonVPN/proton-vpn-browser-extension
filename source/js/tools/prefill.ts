...
const getPmUserValue = async (
	pmUserGetter: PmUserGetter,
	key: string,
	pmUserKey: keyof PmUser,
...
const getPrefillEntries = async (
	...
	(
		await Promise.all(
			fieldsToSeed.map(
				async (key): Promise<readonly [string, string | number] | null> => {
					switch (key) {
						case 'platform':
...
export const getPrefillValues = async (
	...
	return Object.fromEntries(
		await getPrefillEntries(
			pmUserGetter || getPmUserFromBackground,
			fieldsToSeed,
		),
	);
};
